...
10. After the batch: set `Last grounded: YYYY-MM-DD` on `docs/MAINTENANCE.md` and `docs/STATUS.md` only — not on learner guides.
...
15. Before marking a phase guide done, run `python scripts/check_guides.py` from the repo root (stdlib only). It asserts Skeleton-after-Why, The big picture (except concept-only Phase 1), one mermaid, Checkpoint, Try-this placement, banned words, no `#` comments inside python fences, and no session instructions (`Last grounded`, `Fetch before writing`, `Prereq files:`, "A new coding session") in learner-facing files.
