...
- Python topics are taught just-in-time: a short box with example code, inside the guide that first needs the topic (async `run`/`await` in Phase 0; pydantic in Phase 2; TypedDict in Phase 3; pathlib in Phase 4; gather / concurrency in Phase 5; decorators in Phase 7). Each box: one before / one after, a tiny table of what changed, one sentence why this guide needs it. `read, do not paste` means not in the growing `.py` — if the snippet can run, it has prints and an **Expected** block. Do not steal a later phase's punchline. setup.md carries the "Coming from pip?" and "Python you should be comfortable with" sections.
- Learner-facing files (`README.md`, `docs/00-setup.md`, `docs/*-phase-*.md`) must not contain session instructions: no `Last grounded`, `Fetch before writing`, `Prereq files:`, or "A new coding session". Those live in `AGENTS.md` / `docs/STATUS.md` / `docs/MAINTENANCE.md`. Phase prereqs are `Do first:` with learner paths only.
